...
class ClassificationAlgorithms:
    ...
    def feedforward_neural_network(
        ...
        # Apply the model
        pred_prob_train_y = nn.predict_proba(train_X)
        pred_prob_test_y = nn.predict_proba(test_X)
        pred_train_y = nn.predict(train_X)
        pred_test_y = nn.predict(test_X)
        frame_prob_train_y = pd.DataFrame(pred_prob_training_y, columns=nn.classes_)
        frame_prob_test_y = pd.DataFrame(pred_prob_test_y, columns=nn.classes_)

        return pred_train_y, pred_test_y, frame_prob_train_y, frame_prob_test_y
    ...
    def decision_tree(
        ...
        # Do the grid search or fit the model.
        ...
        pred_prob_train_y = dtree.predict_proba(train_X)
        pred_prob_test_y = dtree.predict_proba(test_X)
        frame_prob_train_y = pd.DataFrame(
            pred_prob_train_y, columns=dtree.classes_
        )
